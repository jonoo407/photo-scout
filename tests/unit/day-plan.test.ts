...
// V18: SunCalc rounds to the nearest solar day *by longitude*, so handing it a
// raw early-morning instant resolves to YESTERDAY — every block is already past
// and the day comes back empty. 1 AM is exactly when an astro or blue-hour
// shooter opens the app, so this is a real window, not a boundary curiosity.
describe('dayBlocks — small-hours solar roll (V18)', () => {
  const ONE_AM = new Date(2026, 5, 25, 1, 0)

  it('plans the requested calendar day when asked at 1 AM local', () => {
    const blocks = dayBlocks(ONE_AM, DEFAULT_HOME.lat, DEFAULT_HOME.lng, TZ)
    expect(blocks).toHaveLength(3)
    for (const b of blocks) {
      const z = zoneParts(b.time, TZ)
      expect(`${z.year}-${z.month}-${z.day}`).toBe('2026-6-25')
    }
  })

  it('keeps every block ahead of a 1 AM now, so the day is still buildable', () => {
    const blocks = dayBlocks(ONE_AM, DEFAULT_HOME.lat, DEFAULT_HOME.lng, TZ)
    expect(blocks.filter((b) => b.time > ONE_AM)).toHaveLength(3)
  })

  it('planDay at 1 AM returns a full day rather than an empty one', () => {
    const stops = planDay({
      date: ONE_AM, home: DEFAULT_HOME, spots: SPOTS, timeZone: TZ, now: ONE_AM,
    })
    expect(stops.length).toBeGreaterThanOrEqual(2)
  })

  it('still drops windows that have genuinely passed by late afternoon', () => {
    const fourPM = new Date(2026, 5, 25, 16, 0)
    const stops = planDay({
      date: fourPM, home: DEFAULT_HOME, spots: SPOTS, timeZone: TZ, now: fourPM,
    })
    expect(stops.every((s) => s.block.time > fourPM)).toBe(true)
...
describe('planDay', () => {
  it('returns at least a morning and an evening stop', () => {
    const stops = planDay({ date: DATE, home: DEFAULT_HOME, spots: SPOTS, timeZone: TZ })
    expect(stops.length).toBeGreaterThanOrEqual(2)
  })

  it('never repeats a spot across the day (fixes the 2-spot collapse)', () => {
    const ids = planDay({ date: DATE, home: DEFAULT_HOME, spots: SPOTS, timeZone: TZ }).map((s) => s.spot.id)
    expect(new Set(ids).size).toBe(ids.length)
  })

  it('matches each non-anchor stop to its block’s light', () => {
    for (const s of planDay({ date: DATE, home: DEFAULT_HOME, spots: SPOTS, timeZone: TZ })) {
  ...
  it('honors an anchor: places it exactly once and flags it', () => {
    const stops = planDay({ date: DATE, home: DEFAULT_HOME, spots: SPOTS, timeZone: TZ, anchorId: 'dali-museum' })
  ...
  it('weather verdict favors sheltered and penalizes outdoor categories', () => {
    ...
    const block = dayBlocks(DATE, DEFAULT_HOME.lat, DEFAULT_HOME.lng, TZ).find((b) => b.key === 'sunset')!
  ...
  it('flags stops where weather is a factor, and stays clean on a fair day', () => {
    ...
    const wet = planDay({ date: DATE, home: DEFAULT_HOME, spots: SPOTS, timeZone: TZ, blockWeather: { sunrise: rainy, midday: rainy, sunset: rainy } })
    ...
    const fair = planDay({ date: DATE, home: DEFAULT_HOME, spots: SPOTS, timeZone: TZ, blockWeather: { sunrise: clear, midday: clear, sunset: clear } })
    expect(fair.every((s) => !s.weather)).toBe(true)
  })

  it('rankForBlock re-ranks candidates by proximity to the current origin', () => {
    const sunset = dayBlocks(DATE, DEFAULT_HOME.lat, DEFAULT_HOME.lng, TZ).find((b) => b.key === 'sunset')!
...
describe('planDay — never schedules a window that already passed (UI-critic major)', () => {
  it('drops the morning block when building mid-morning', () => {
    const blocks = dayBlocks(DATE, DEFAULT_HOME.lat, DEFAULT_HOME.lng, TZ)
    const midMorning = new Date(blocks[0].time.getTime() + 3 * 3600_000) // 3h after golden peak
    const stops = planDay({ date: DATE, home: DEFAULT_HOME, spots: SPOTS, timeZone: TZ, now: midMorning })
    expect(stops.some((s) => s.block.key === 'sunrise')).toBe(false)
  ...
  it('keeps every block when building before dawn', () => {
    ...
    const stops = planDay({ date: DATE, home: DEFAULT_HOME, spots: SPOTS, timeZone: TZ, now: predawn })
...
describe('pinPlan (saved/shared plans — fixed stops, no re-planning)', () => {
  ...
  it('renders exactly the pinned spots, in block time order, with drive legs', () => {
    const stops = pinPlan({ date: DATE, home: DEFAULT_HOME, spots: SPOTS, timeZone: TZ, stops: REFS })
  ...
  it('skips ids that are not in the loaded spots instead of crashing', () => {
    const stops = pinPlan({
      date: DATE, home: DEFAULT_HOME, spots: SPOTS, timeZone: TZ,
