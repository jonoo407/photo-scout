...
3. ~~**Deploy**~~ — DONE. Currently live at **https://jonoo407.github.io/photo-scout/** via GitHub Pages + Actions (`.github/workflows/deploy.yml`, auto-deploys on push to `main`). Repo: https://github.com/jonoo407/photo-scout (public). Note: spot photos hotlink `upload.wikimedia.org` (online-only; SW doesn't cache them). **Migrating to Cloudflare Pages at `shootvantage.com`** (domain owned on Cloudflare): connect the repo in Cloudflare Pages (build `npm run build`, output `dist`, Node pinned via `.node-version`=22), add `shootvantage.com` as the custom domain (auto DNS + SSL). App is root-domain-ready already (`base: './'` + hash router; no code change). Retire the GH Pages workflow once Cloudflare is verified.
3b. **Accounts + sync** — CODE DONE (2026-07-02), awaiting Supabase project. Passwordless magic-link sign-in (Supabase, PKCE `?code=` flow — compatible with the hash router) + cross-device sync of wishlist/visited/checklist/home/region/prefs into `vantage_state` (RLS own-row; `supabase/schema.sql`). Fully env-gated: until `VITE_SUPABASE_URL`/`VITE_SUPABASE_ANON_KEY` are set (Cloudflare Pages → Settings → Environment variables; `.env.local` for dev) the Account section is hidden and supabase-js isn't even in the bundle (dead-code eliminated; configured builds emit it as a lazy ~55KB-gzip chunk). Merge on sign-in: lists union, account scalars win (`src/auth/sync-merge.ts`). Setup: create project at database.new → Connect panel for URL + publishable key → run schema.sql in SQL Editor → set Site URL (Authentication → URL Configuration) to the app origin.
