...
14. **In-app feedback / feature requests** (added 2026-07-06) — gather user feedback with a light touch: (a) a permanent "Send feedback" row on the Settings screen (short form: what's working / what's missing, kind = feedback|feature, optional email if signed-out user wants a reply); (b) a **monthly, non-annoying nudge** — one small dismissible card (Today screen), at most once per 30 days per device (`feedbackPromptAt` in the persisted store), never a modal, never during onboarding, only after real usage (e.g. 3+ sessions), and both submit AND dismiss reset the 30-day clock; (c) rows accumulate in an insert-only `feedback` table (same RLS pattern as `spot_suggestions` — no public reads); (d) periodic review sessions: the user asks Claude to pull + summarize accumulated feedback via SQL and turn the good ones into backlog items.

## OPEN BACKLOGS (restructured 2026-07-06) — the current view; split by who's needed

Everything above this section is historical record (all shipped). These two lists are what's left.

### A. Needs the human (Jon)

1. **iOS App Store deployment** — Capacitor wrap + cloud CI (Codemagic free tier / Capawesome; NOT Appflow — sunsetting). Claude does all code + CI config; Jon does: Apple Developer Program enrollment ($99/yr, payment), App Store Connect agreement clicks, and signing-key approval. Includes swapping web push → native APNs in the wrapper (Worker gains an APNs sender). Interim: the PWA installs from Safari today with push working.
2. **Design handoff round-trip** — hand the UI to the cloud design tool for a polish pass, import it back; plus custom graphics beyond what it does natively (hero illustrations, empty-state art, og images — possibly via image-generation engines). Human: runs the handoff in claude.ai, makes the taste calls, approves what comes back.
...
1. **#14 Feedback gathering** — Settings entry + gentle monthly nudge + accumulate for review sessions (spec in item 14 above).
2. **Every spot ships with a photo (75/75)** — st-paul-ame is the one gap. Order of attack: exhaust REAL sources first (Commons search-term variants, Openverse, Flickr CC — the pipeline covers all allowed hosts), and only if genuinely nothing exists consider a clearly-labeled AI-generated placeholder image as the fallback. Also: keep `fix-media-hashes` + `verify-media-urls` green (full-catalog validation last run 2026-07-06).
3. **Auth-gate the app + guest accounts** — require sign-in up front: unauthenticated users land on a login screen; two fast paths: Google SSO (once A3 is done) or an instant no-friction **guest account** (Supabase anonymous sign-in — enable via Management API; upgradeable to a real email account later, data intact). ⚠️ Product note: this reverses the current local-first/no-account-needed design and the onboarding flow — confirm the tradeoff at build time (client `/l/` + `#/list` links must STAY account-free for clients).
4. **Playwright e2e + axe a11y** at iPhone viewport (config scaffolded, no suite yet).
5. **Magic Layers** — rocket-launch calendar (Launch Library 2, keyless) + birding overlay (eBird — free key, 2-min form = tiny human assist if wanted). Tides already shipped in Best Days.
6. **Golden-hour reminders** — daily "golden hour in 40 min" ping; decide overlap vs Conditions alerts first.
...
12. **"Vote for the next city" scoreboard** (added 2026-07-06): a fun, gamified public scoreboard of candidate cities — live vote bars, ranks, playful copy ("Austin needs 12 votes to overtake Denver"). One vote per account (guest accounts OK once B3 lands); `city_votes` table, candidate list data-driven. **"Invite friends to vote" button** — share link that lands on the scoreboard with your city pre-championed (and awards B11 points when the invite converts). The winner literally becomes the next curation target (city #3 → triggers the SCALING.md tier work). Surface: city picker in Settings + a card on Browse when the user's region hunger is detectable (e.g. searched a city we don't have).
